...
export default function CheckMyPermanenceDetails({
  ...
  const membres = [
    ...
    ...permanence.equipe.Soldats.map((s) => ({
      ...s,
      // ✅ Condition sur le grade
      roleEquipe: s.grade?.toLowerCase() === "soldat" ? "Soldat" : "Chef de Poste",
    })),
  ];
  const membresValides = membres.filter(p => p.Matricule);
  const getHoraires = (perm: PermanenceCheck) => {
    if (perm.horaire && perm.horaire.length > 0) {
      return perm.horaire.map((h) => ({
    ...
    return [
      {
        affiche: `${formatDateFR(perm.debut)} - ${formatDateFR(perm.fin)}`,
      },
    ];
  };
  // ✅ Récupérer les horaires une seule fois pour toute la permanence
  const horairesPermanence = getHoraires(permanence);
  return (
        ...
        <table className="table table-zebra w-full min-w-225">
          <thead className="bg-gray-100 text-gray-700 sticky top-0 z-10">
            <tr>
              ...
              <th className="text-left px-3 py-2">Horaires</th>
            </tr>
          </thead>

          <tbody>
        
            {membresValides.map(p => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-center">
                    {/* ✅ Utiliser les horaires de la permanence pour tous */}
                    {horairesPermanence.map((h, i) => (
