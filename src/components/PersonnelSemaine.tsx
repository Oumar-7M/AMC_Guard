...
// Fonction utilitaire pour formater la date comme dans le PDF source (ex: "26/02 16:30")
const formaterDate = (dateString: string) => {
  ...
  return date.toLocaleString("fr-FR", {
    ...
    hour: "2-digit",
    minute: "2-digit",
  }).replace(" à", ""); // Enlève le "à" pour gagner de la place
};

export default function PersonnelSemaine({ personnelsParGrade, semaine }: Props) {
  ...
  return (
    <div className="min-h-screen flex flex-col p-6 bg-slate-50 print:bg-white print:p-4 font-sans">
      ...
      {/* VERSION DESKTOP & PRINT */}
      <div className="hidden md:block space-y-10 w-full">
        {grades.map((grade) => {
          const personnes = personnelsParGrade[grade];

          return (
            <div key={grade} className="w-full page-break-inside-avoid print:mb-6">
              <table className="w-full border-collapse border-2 border-slate-800 print:border-black bg-white">
                <thead>
                  {/* LIGNE 1 : LE GRADE (Intégré au tableau pour un alignement parfait) */}
                  <tr className="bg-slate-800 print:bg-gray-200 text-white print:text-black">
                    <th colSpan={8} className="px-4 py-2 border-b-2 border-slate-800 print:border-black">
                      <div className="flex justify-between items-center">
                        <span className="font-bold uppercase tracking-wider text-sm print:text-[13px]">
                          {grade}
                        </span>
                  ...
                  {/* LIGNE 2 : LES EN-TÊTES DE COLONNES */}
      ...
      {/* VERSION MOBILE (Design en cartes optimisé) */}
      <div className="md:hidden space-y-6">
        {grades.map((grade) => {
          const personnes = personnelsParGrade[grade];

          return (
            <div key={grade} className="bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
              <div className="bg-slate-800 text-white px-4 py-3 flex justify-between items-center">
                <h2 className="font-bold text-sm uppercase tracking-wider">{grade}</h2>
                <span className="bg-white text-slate-800 text-xs px-2 py-1 rounded-full font-bold">
                  {personnes.length} pers.
                </span>
              </div>
                ...
                {personnes.map((p) => (
                  <div key={p.Matricule} className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="font-bold text-lg text-slate-800 uppercase">{p.Nom} <span className="capitalize font-medium text-slate-600">{p.Prenom}</span></p>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm text-slate-600 mb-3 border-y py-2 mt-2">
                      <p><span className="font-semibold block text-xs uppercase text-slate-400">Arme</span> {p.Arme}</p>
                      <p><span className="font-semibold block text-xs uppercase text-slate-400">Téléphone</span> {p.numero}</p>
                    </div>

                    <div className="flex justify-between text-xs font-mono">
                      <div className="bg-green-50 text-green-700 px-2 py-1 rounded border border-green-100">
                        <span className="font-bold">Début:</span> {formaterDate(p.DateDebut)}
                      </div>
                      <div className="bg-red-50 text-red-700 px-2 py-1 rounded border border-red-100">
                        <span className="font-bold">Fin:</span> {formaterDate(p.DateFin)}
                      </div>
                    </div>
                  </div>
                ))}
